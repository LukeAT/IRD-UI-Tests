...
test.describe('Outright test suite', () => {
    ...
    test.beforeAll(async ({ browser }) => {
        ...
        //Sign in to bid.
        await bs.signIn(bsPage, auth.outright.bs.username, auth.INF.bs.password)
        await ss.signIn(ssPage, auth.outright.ss1.username, auth.INF.ss1.password)
    ...
    test('OUT Send INF shortcode and verify rfq status after ss acknowledges', async () => {
    ...
    test(`OUT upload outright TSV and verify rfq status after ss acknowledges`, async () => {
    ...
    for (let i = 0; i < shortcodes.length; i++) {
        ...
        test(`OUT Send shortcodes and verify rfq status after ss acknowledges ${i}`, async () => {
