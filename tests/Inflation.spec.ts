...
test.describe('INF Verify details for outright swaps', async () => {
  // Use soft assertions.
  ...
  for (const p of params) {
    test(`INF Send csv param shortcodes and verify details after affirm. ${p.key}`, async () => {
  ...
  for (const p of params1) {
    test(`INF Send csv params csv RFQ's and verify details after affirm. ${p.key}`, async () => {
