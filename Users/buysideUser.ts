...
export default class BuysideUser extends BasePage {
  ...
  static async Setup(browser: Browser, credentials: { username: string; password: string }): Promise<BuysideUser> {
    const user = (await super.Setup(browser, credentials)) as BuysideUser;

    await user.archiveAll();
    return user;
  }

  async archiveAll() {
    await this.page.goto('/api/bid/archiveallthethingsquickly');
  }
  ...
  async sendsRFQ(options?: { bank2?: string; oneWay?: boolean; withDeltaX?: boolean; dxNot?: string; atmFr?: string }) {
    ...
    // Apply options.
    if (options?.bank2 !== undefined) {
      await this.page.getByRole('button').filter({ hasText: options.bank2 }).click();
    }
    if (options?.oneWay === true) {
      await this.oneWay().click();
    }
    if (options?.withDeltaX === true) {
      await this.withDeltaExchange().check();
    }
    if (options?.dxNot !== undefined) {
      await this.dxNotional().fill(options.dxNot);
    }
    if (options?.atmFr !== undefined) {
      await this.atmForwardRate().fill(options.atmFr);
    }
