...
export default class SellsideUser extends BasePage {
  ...
  static async Setup(browser: Browser, credentials: { username: string; password: string }): Promise<SellsideUser> {
    return super.Setup(browser, credentials) as Promise<SellsideUser>;
  }
  ...
  async entersDetails(options?: { dxDir?: string }) {
    ...
    // Choose DX Direction.
    if (options?.dxDir !== undefined) {
      await this.dmDxDirDropDown().selectOption(options.dxDir);
    }
